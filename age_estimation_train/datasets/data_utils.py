...
def get_train(n, transform):
    ...
    t = n.get('appa_real', 0)
    if t is None or t > 0:
        appa_tr = AppaRealDataset(subset='train/', transform=transform, n=t, ext='', crop_faces=False)
        todo.append(appa_tr)
...
def get_validation(n, transform, to_split=None):
    ...
    t = n.get('appa_real', 0)
    if t is None or t > 0:
        appa_val = AppaRealDataset(subset='val/', transform=transform, n=t, ext='', crop_faces=False)
