...
def get_label(image_loss, labels):
    ...
    elif image_loss.__name__ == gaussian_kl_divergence.__name__:
        gt = labels['app_age'].to(DEVICE, dtype=torch.float32), \
            labels['app_age_std'].to(DEVICE, dtype=torch.float32)
    else:
